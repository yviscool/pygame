...
class Fish(Sprite):
    ...
    def __init__(self):
        ...
        self.rect.topleft = (
            random.uniform(*xInfo),
            random.uniform(*yInfo)
        )

        self.min_angle = random.uniform(*minAngleInfo)
        self.max_angle = random.uniform(*maxAngleInfo)
    ...
    def change_angle(self):
        ...
        # print(self.angle, self.min_angle, self.max_angle)
    ...
    def move_forward(self):
        ...
        if angle > 0 and angle < 90:
            ...
            dx = math.sin(math.radians(angle-90)) * self.speed
            dy = math.cos(math.radians(angle-90)) * self.speed
        elif angle > 180 and angle < 270:
            dx = math.cos(math.radians(angle-180)) * self.speed
            dy = math.sin(math.radians(angle-180)) * self.speed
        elif angle > 270 and angle < 360:
            dx = math.sin(math.radians(angle-270)) * self.speed
            dy = math.cos(math.radians(angle-270)) * self.speed
...
pygame.time.set_timer(PRODUCT_FISH_EVENT, 1000 * 10)
