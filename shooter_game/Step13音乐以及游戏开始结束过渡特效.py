...
def draw_bg():
	...
	# screen.blit(sky_img, (0 - bg_scroll, 0))
	# screen.blit(mountain_img, (0 - bg_scroll, SCREEN_HEIGHT - mountain_img.get_height() - 300))
	# # 松树
	# screen.blit(pine1_img, (0 - bg_scroll, SCREEN_HEIGHT - pine1_img.get_height() - 150))
	# # 松树
	# screen.blit(pine2_img, (0 - bg_scroll, SCREEN_HEIGHT - pine2_img.get_height()))
	...
	def move(self, move_left, move_right):
		...
		for tile in world.obstacle_list:
			...
			if tile[1].colliderect(self.rect.x + dx, self.rect.y, self.width, self.height):
				...
				# 控制敌人 ai 碰到墙壁时候转向, 因为敌人是移动到某段距离的时候转向
			...
			if tile[1].colliderect(self.rect.x, self.rect.y + dy, self.width, self.height):
				...
				if self.vel_y < 0:
					self.vel_y = 0
					dy = tile[1].bottom - self.rect.top # 要移动的距离
		...
		# check if going off the edges off th screen
		...
		return screen_scroll, level_comlete
...
class World():
	...
	def process_data(self, data):
		...
		# iterate throught each value in level datafile
...
class Explosion(pygame.sprite.Sprite):
	...
	def update(self):
		...
		if self.counter >= EXPLOSION_SPEED:
			self.counter = 0
			self.frame_index += 1
			# if the animation is complete then detele the expolotion
...
# ammo_box = ItemBox('Ammo', 400, 350)
# grenade_box = ItemBox('Grenade', 600, 350)
# item_box_group.add(item_box)
# item_box_group.add(ammo_box)
# item_box_group.add(grenade_box)

# player = Soldier('player', 200,  200, 1.651, 5, 20, 5)
# health_bar = HealthBar(10, 10, player.health, player.max_health)

# enemy = Soldier('enemy', 500,  300, 1.65, 2, 20,  0)
...
while run:
	...
	if not start_game:
		...
		if player.alive:
			...
			screen_scroll, level_comlete = player.move(move_left, move_right)
