...
# 		# update image deending on current action
# 		self.image = self.animation_list[self.action][self.frame_index]
# 		# check if enough time has passed since the last update
# 		if pygame.time.get_ticks() - self.update_time > ANIMATION_COOLDOWN:
# 			self.frame_index += 1
...
# 		# if the animation has run out then reset back to te start
# 		if self.frame_index >= len(self.animation_list[self.action]):
# 			self.frame_index = 0
...
# 		self.animation_list = animation_list
# 		self.frame_index = 0
# 		self.action = 0 # 0: walk 1:attack 2: dealth
# 		self.update_time = pygame.time.get_ticks()
...
# 	def update(self, surface, castle, bullet_group):
...
# 			# check if enemy has reached the castleo
...
# 			# check if healthis dropped to zero
# 			if self.health <= 0:
# 				castle.money += 100
# 				castle.score += 100
# 				self.update_action(2) # death
...
# 	def update_action(self, new_action):
# 		# check if the new aciton is different to the previous action
# 		if new_action != self.action:
# 			self.action = new_action
...
# 		self.speed = speed
# 		self.health = health
# 		self.last_attck = pygame.time.get_ticks()
# 		self.attack_cooldown = 1000
# 		self.animation_list = animation_list
# 		self.frame_index = 0
# 		self.action = 0 # 0: walk 1:attack 2: dealth
...
# 			# attack
# 			if self.action == 1:
# 				# check if enough time has passed since llast attack
# 				if pygame.time.get_ticks() - self.last_attck > self.attack_cooldown:
# 					castle.health -= 25
# 					if castle.health <= 0:
# 						castle.health = 0
# 					self.last_attck = pygame.time.get_ticks()
